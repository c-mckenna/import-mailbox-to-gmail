...
from googleapiclient.http import MediaIoBaseUpload, set_user_agent
...
def process_mbox_files(username, service, labels, http_factory):
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for file in files:
            ...
            def process_message(local, index, message):
                ...
                try:
                    ...
                    message_response = service.users().messages().import_(
                        ...
                        media_body=media).execute(http=local.http, num_retries=args.num_retries)
            ...
            def initializer_worker(local_context):
                local_context.http = http_factory()

            with ThreadPoolExecutor(initializer=initializer_worker, initargs=(local,), max_workers=10) as thread_pool:
                for result in thread_pool.map(lambda arguments: process_message(local, *arguments), enumerate(mbox)):
                    if result:
...
def main():
    ...
    def http_factory():
        return credentials.authorize(set_user_agent(
                    httplib2.Http(),
    ...
    for username in next(os.walk(args.dir))[1]:
        try:
            ...
            try:
                result = process_mbox_files(username, service, labels, http_factory)
            except Exception:
